...
extern volatile uint8_t uart_debug_cmd_index;
void enter_normal_app_mode(void)
{
    while(1)
    {
        ...
        if(uart_debug_cmd_index)///for test
        {
            uart_printf("uart_debug_cmd_index = %x\r\n", uart_debug_cmd_index);
            uint16_t send_handle=0;
            uint16_t send_uuid=0xfee2;
            uint8_t send_data[4]={0x55,0x66,0x77,0x88};
            send_handle=sdp_get_handle_from_uuid(uart_debug_cmd_index-1,2,&send_uuid);
            uart_printf("send_handle = %x\r\n", send_handle);
            appc_write_service_data_req(uart_debug_cmd_index-1,send_handle,4,send_data);
            
            uart_debug_cmd_index=0;
