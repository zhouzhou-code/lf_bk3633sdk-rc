                                           ...
                                           ke_task_id_t const src_id)
{
    ...
    if (app_env.conidx != GAP_INVALID_CONIDX)
    {
        ...
        ke_timer_set(APP_GATTC_SDP_TIMER,TASK_APP,10);
    ...
    switch(param->operation)
    {
        ...
        case (GAPC_ENCRYPT):
        {
            ...
            else
            {
                ...
                if(sdp_env_init.sdp_all_profiles_enable==0)
                {
                    ke_state_set(KE_BUILD_ID(TASK_APP,app_env.conidx), APPC_SDP_DISCOVERING);
                    sdp_discover_all_service(app_env.conidx);
                }
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    if(ke_timer_active(APP_GATTC_SDP_TIMER,TASK_APP))
        ke_timer_clear(APP_GATTC_SDP_TIMER,TASK_APP);
                                     ...
                                     ke_task_id_t const src_id)
{
   ...
   // struct prf_sdp_db_env *prf_db_env = NULL;
    ...
    if(ke_timer_active(APP_DISCOVER_END_TIMER,TASK_APP))
        ke_timer_clear(APP_DISCOVER_END_TIMER,TASK_APP);
    
    ke_timer_set(APP_DISCOVER_END_TIMER,TASK_APP,200);
...
static int app_dsicover_end_handler(ke_msg_id_t const msgid,struct gapm_profile_added_ind *param,
                                    ke_task_id_t const dest_id,ke_task_id_t const src_id)
{
    uart_printf("~~~~~~~~~~~~~~~~~~~~~%s\r\n", __func__);
    uint8_t conidx = KE_IDX_GET(dest_id);
    if((ke_state_get(KE_BUILD_ID(TASK_APP,conidx)) == APPC_SERVICE_CONNECTED))
        sdp_env_init.sdp_all_profiles_enable=1;
...
KE_MSG_HANDLER_TAB(appm)
{
    ...
    {APP_DISCOVER_END_TIMER,      (ke_msg_func_t)app_dsicover_end_handler},    
