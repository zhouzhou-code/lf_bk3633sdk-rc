    ...
    if(state == SCPPC_IDLE)
    {
        ...
        else
        {
            ...
            if(handle != ATT_INVALID_HANDLE)
            {
                ...
                prf_gatt_write(&scppc_env->prf_env, conidx, handle, data, 2*sizeof(uint16_t), GATTC_WRITE_NO_RESPONSE,0);
    ...
    if(state == SCPPC_IDLE)
    {
        ...
        else
        {
            ...
            if(handle != ATT_INVALID_HANDLE)
            {
                status = GAP_ERR_NO_ERROR;
                // Send GATT Write Request
                prf_gatt_write_ntf_ind(&scppc_env->prf_env, conidx, handle, param->ntf_cfg,0);
                // store context of request and go into busy state
