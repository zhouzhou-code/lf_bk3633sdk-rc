...
void R_RX_PAYLOAD(uint8_t numb);
void W_TX_PAYLOAD(uint8_t numb);
void FLUSH_TX(void);
void FLUSH_RX(void);
void REUSE_TX_PL(void);
void ACTIVE(void);
uint8_t R_RX_PL_WD(void);
void W_ACK_PAYLOAD(uint8_t numb, uint8_t pipe);
void W_TX_PAYLOAD_NOACK(uint8_t numb);
void NOP(void);
void W_ANALOG_REG(uint8_t addr, uint32_t dreg);
void bk2401_initial(void);
void test_bk24(uint8_t* para);
void goto_sleep();
